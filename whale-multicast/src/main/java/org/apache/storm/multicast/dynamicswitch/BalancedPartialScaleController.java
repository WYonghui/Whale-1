...
public class BalancedPartialScaleController {
    public static GraphAndChanges change(DirectedAcyclicGraph<String, DefaultEdge> graph, int oldDegree, int newDegree) {
        if (oldDegree > newDegree) {
            return scaleDown(graph, newDegree);
        } else if (oldDegree < newDegree) {
            return scaleUp(graph, newDegree);
        } else {
            return new GraphAndChanges(graph, new HashMap<>());
        }
    }

    private static GraphAndChanges scaleDown(DirectedAcyclicGraph<String, DefaultEdge> graph, int newDegree) {
        Set<String> oldVertexSet = graph.vertexSet();
        String root = oldVertexSet.iterator().next();
        if (graph.outDegreeOf(root) <= newDegree)
            return new GraphAndChanges(graph, new HashMap<>());

        HashMap<String, ChangeMsg> changes = new HashMap<>();
        ...
        while (!dfiQueue.isEmpty()) {
            ...
            while (iter.hasNext()) {
                ...
                changes.put(rmSplit[0], new ChangeMsg().setDisconnect(vSplit[0]));
                removeEdges.add(t);
            }
                ...
                if (oldEdges != null && oldEdges.hasNext()) {
                    ...
                    if (Integer.parseInt(split[1]) != id || Integer.parseInt(split[2]) != layer || Integer.parseInt(split[3]) != layerIndex) {
                        if (changes.containsKey(split[0])) {
                            ...
                            changes.put(split[0], new ChangeMsg().setMeta(id, layer, layerIndex));
                        }
                    }
                    ...
                    graph.outgoingEdgesOf(oldV).forEach(e -> {
                        ...
                        changes.put(split[0], new ChangeMsg().setDisconnect(oSplit[0]));
        ...
        return new GraphAndChanges(newGraph, changes);
    }

    private static GraphAndChanges scaleUp(DirectedAcyclicGraph<String, DefaultEdge> graph, int newDegree) {
        HashMap<String, ChangeMsg> changes = new HashMap<>();
        DirectedAcyclicGraph<String, DefaultEdge> newGraph = new DirectedAcyclicGraph<>(DefaultEdge.class);
            ...
            while (iter.hasNext()) {
                ...
                if (oldEdges != null && oldEdges.hasNext()) {
                    ...
                    String parentId = graph
                            ...
                            .split("-")[0];
                    changes.put(vCutFromEnd[0], new ChangeMsg()
